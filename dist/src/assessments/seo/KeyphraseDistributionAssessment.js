...
class KeyphraseDistributionAssessment extends _assessment2.default {
	constructor(config = {}) {
		super();
	...
	calculateResult(i18n) {
		...
		if (distributionScore === 100) {
			return {
				...
				resultText: i18n.sprintf(i18n.dgettext("js-text-analysis", "%1$sKeyphrase distribution%3$s: " + "%2$sInclude your keyphrase or its synonyms in the text so that we can check keyphrase distribution%3$s."), this._config.urlTitle, this._config.urlCallToAction, "</a>")
			};
		}
		...
		if (distributionScore > this._config.parameters.acceptableDistributionScore) {
			return {
				...
				resultText: i18n.sprintf(i18n.dgettext("js-text-analysis", "%1$sKeyphrase distribution%3$s: " + "Very uneven. Large parts of your text do not contain the keyphrase or its synonyms. %2$sDistribute them more evenly%3$s."), this._config.urlTitle, this._config.urlCallToAction, "</a>")
			};
		}
		...
		if (distributionScore > this._config.parameters.goodDistributionScore && distributionScore <= this._config.parameters.acceptableDistributionScore) {
			return {
				...
				resultText: i18n.sprintf(i18n.dgettext("js-text-analysis", "%1$sKeyphrase distribution%3$s: " + "Uneven. Some parts of your text do not contain the keyphrase or its synonyms. %2$sDistribute them more evenly%3$s."), this._config.urlTitle, this._config.urlCallToAction, "</a>")
			};
		}

		return {
			score: this._config.scores.good,
			resultText: i18n.sprintf(i18n.dgettext("js-text-analysis", "%1$sKeyphrase distribution%2$s: Good job!"), this._config.urlTitle, "</a>")
		};
	}
